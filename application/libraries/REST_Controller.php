 ...
 * @version         3.0.0
 */
abstract class REST_Controller extends \FMS_Controller {
    ...
    protected function _detect_method()
    {
        ...
        if ($this->config->item('enable_emulate_request') === TRUE)
        {
            ...
            $method = isset($method) && strtolower($method);
        }
    ...
    protected function _detect_api_key()
    {
        ...
        $this->response([
            $this->config->item('rest_status_field_name') => FALSE,
            $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_invalid_api_key')
        ], self::HTTP_UNAUTHORIZED);
        $this->output->_display();
        exit;
        return FALSE;
    ...
    {
        $payload['rtime'] = isset($this->_end_rtime) - isset($this->_start_rtime);
